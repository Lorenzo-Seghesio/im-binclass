...
def nested_cv(X, y, trial, outer_splits=3, inner_splits=3):
    ...
    for train_val_idx, test_idx in outer_cv.split(X, y):
        ...
        for train_idx, val_idx in inner_cv.split(X_train_val, y_train_val):
            ...
            train_loader = DataLoader(train_ds, batch_size=trial.params['batch_size'], shuffle=True)
            val_loader = DataLoader(val_ds, batch_size=trial.params['batch_size'])

            model = SimpleBinaryClassifier(input_dim=X.shape[1], hidden_size=trial.params['hidden_size']).to("cuda" if torch.cuda.is_available() else "cpu")
            criterion = BinaryFocalLoss(alpha=trial.params['alpha'], gamma=trial.params['gamma'])
            optimizer = torch.optim.Adam(model.parameters(), lr=trial.params['lr'])

            model = train_one_fold(model, train_loader, val_loader, torch.device("cuda" if torch.cuda.is_available() else "cpu"), criterion, optimizer)

            model.eval()
            with torch.no_grad():
                val_preds = torch.sigmoid(model(torch.tensor(X_val, dtype=torch.float32).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))))
                val_preds = (val_preds > 0.5).float().cpu().numpy()
        ...
        best_model.eval()
        with torch.no_grad():
            test_preds = torch.sigmoid(best_model(torch.tensor(X_test, dtype=torch.float32).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))))
            test_preds = (test_preds > 0.5).float().cpu().numpy()
            outer_f1 = f1_score(y_test, test_preds)
        f1_scores.append(outer_f1)
